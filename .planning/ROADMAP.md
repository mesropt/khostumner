...
- [x] **Phase 5: Promise Submission** - Registered users submit and edit promises; election linking (completed 2026-05-24)
...
**Wave 1 (blocking)**
- [x] 05-01-PLAN.md — Wave 0 scaffold: promise_edits migration + ORM models + Pydantic schemas + npm/pip deps + RED test stubs

**Wave 2 (parallel after Wave 1)**
- [x] 05-02-PLAN.md — Backend endpoints: POST /api/promises + PUT /api/promises/{slug} + account_age_days in UserRead
...
**Wave 4 (after Wave 3)**
- [x] 05-05-PLAN.md — PromiseEditPage (/promises/:slug/edit) + edit link on PromiseDetailPage + human verification

**Cross-cutting constraints:**
- moderation_status SAEnum reuse: promise_edits migration uses sa.Enum(create_type=False); ORM imports from app.models.promises
- account_age_days is login-snapshot (not real-time) — accepted v1 limitation
- PUT /api/promises/{slug} looks up any non-rejected promise (not approved-only)
- promise_edits rows are append-only — never DELETE or UPDATE in Phase 5
**UI hint**: yes
...
| 5. Promise Submission | 5/5 | Complete   | 2026-05-24 |
